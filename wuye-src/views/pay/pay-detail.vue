	...
	<div>
		...
		<!-- 用户信息 -->
			...
			<form class="invoice">
				...
				<div class="form-row" v-if="invoiceType == 'company'&&needInvoice=='yes' ">
					<mt-field label="发票抬头" placeholder="发票抬头" v-model="invoiceTitle"></mt-field>
				</div>
				<div class="form-row" v-if="invoiceType == 'company'&&needInvoice=='yes' ">
					<mt-field label="公司税号" placeholder="公司税号" v-model="creditCode"></mt-field>
				</div>
			</form>
			<!-- 支付按钮 -->
	  ...
	  //       })
		...
		methods:{
			...
			wechatPay(){
				...
				let url = "getPrePayInfo?billId="+vm.routeParams.billIds+"&stmtId="+vm.routeParams.stmtId+"&couponUnit="+vm.upronAmountNumber+"&couponNum=1&couponId="+vm.couponId+"&mianBill="+vm.mianBill+"&mianAmt="+vm.mianAmt+"&reduceAmt="+vm.reduceAmt+"&invoice_title_type="+''+"&credit_code="+''+"&invoice_title="+'';
