		...
		<mt-tab-container v-model="selected">
		  ...
		  <mt-tab-container-item id="d">
		  	...
		  	<div class="query-data">
		  		<div class="input-row">
			  		小区： 
			  		<select class="virtual-input"  v-model="sectqq" v-on:change="sectSelect($event)">
			  			<!-- <option value="" disabled>请选择小区</option> -->
			  			<option v-for="(item,index) in sectList" :value="item.id" >{{item.name}}</option>	
			  			
			  		</select>
			  	</div>
			  	<div class="input-row">
			  		楼宇：
			  		<select class="virtual-input" v-model="buildqq" v-on:change="buildSelect($event)">
			  			<!-- <option value="" disabled>请选择楼宇</option> -->
			  			<option v-for="(item,index) in buildList" :value="item.id" >{{item.name}}</option>	
			  		</select>
			  	</div>
			  	<div class="input-row">
			  		门牌：
			  		<select class="virtual-input" v-model="unitqq" v-on:change="unitSelect($event)">
			  			<!-- <option value="" disabled>请选择门牌</option> -->
			  			<option v-for="item in unitList" :value="item.id" >{{item.name}}</option>	
			  		</select>
			  	</div>
			  	<div class="input-row">
			  		室号：
			  		<select class="virtual-input" v-model="houseqq" v-on:change="houseSelect($event)">
			  			<!-- <option value="" disabled>请选择室号</option> -->
			  			<option v-for="item in houseList" :value="item.id">{{item.name}}</option>	
			  		</select>
			  	</div>	
		  	</div>
...
<script>
   ...
   //  			vm.queryBillList();
	  ...
	  methods:{
	  	sectSelect(event){
	  		vm.sectqq = event.target.value;
	  		
	  		vm.getCellMng(vm.sectqq, "", "", '03');//楼宇 

	  	},
	  	buildSelect(event){
	  		vm.buildqq = event.target.value;
	  		vm.getCellMng(vm.sectqq,vm.buildqq,'','02');
	  	...
	  	unitSelect(event){
	  		...
	  		vm.unitqq = event.target.value;
	  		vm.getCellMng(vm.sectqq,vm.buildqq,vm.unitqq,'01');
	  	},
	  	houseSelect(event){
	  		vm.queryBillInfo= [];//清空查询账单列表
    		vm.queryBillPage = 1;//页码重置
	  	...
	  	getCellMng(sect_id,build_id,unit_id,data_type){
			...
			vm.receiveData.getData(vm,url,'queryInfo',function(){
				...
				if ("03"==data_type) {
				 	...
				 	console.log(vm.buildList.length==0)
				 	if(vm.buildList.length==0){
				 		// alert(1)
				 		console.log(vm.sectqq);
				 		vm.getCellMng(vm.sectqq, "0", "", '02');//门牌
				 	}
				 	vm.buildqq = '';
		  			vm.unitqq = '';
		  			vm.houseqq = '';
					...
					if(vm.unitList.length==0){
			  			vm.getCellMng(vm.query.sect,vm.buildqq,'0','02');
			  		}
			  		vm.unitqq = '';
			  		vm.houseqq = '';
					vm.houseList = [];
		...
		queryBillList(){
			...
			vm.receiveData.getData(vm,url,'queryBillInfoData',function(){
				...
				console.log(vm.queryBillInfoData.result)
